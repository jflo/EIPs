...
This workaround exists because a single, identifiable party can unilaterally cause non-reveal. "Threshold decryption via silently-keyed committees" under Alternative specifications describes a design with no such single liable party, removing the need for sponsor-ST liability altogether.
...
A more structural alternative is to remove the identifiable key publisher altogether. "Threshold decryption via silently-keyed committees" under Alternative specifications sketches a design in which each ST's decryptors are a validator committee hidden until after commitment, so there is no specific, pre-identifiable key publisher for a PTC coalition to target.
...
* The single-key bundles can be useful for more elaborate schemes, for example under threshold decryption (see "Threshold decryption via silently-keyed committees" below).
...
#### Threshold decryption via silently-keyed committees

The baseline design gives a single party — either a chosen `key_publisher` or a self-decrypting sender — full discretion over whether to release a `LucidKeyMessage`. The ToB fee prices this discretion (see "Probabilistic front-running", point 4) but does not remove it. Two residual issues, both already acknowledged elsewhere in this EIP, follow from the same root cause: a single, identifiable party controls each ST's release.

* As noted under "Exogenous MEV", a decryptor can selectively withhold or release a key once it observes information that arrives after commitment, extracting a narrow secondary information-ingress boundary.
* As noted under "PTC influence and state observation", a PTC coalition can be bribed to systematically extend the release window for a specific, known key publisher's keys, since key-timeliness votes are cast per ST-commitment against a named party.

[Hall-Andersen, Simkin, and Wagner](https://eprint.iacr.org/2025/1384) describe a *silent threshold encryption* scheme that removes this single point of discretion structurally, rather than only economically. Every validator generates a keypair once, independently and out of band — no interactive ceremony, no distributed key generation, no dependency on which STs will later be encrypted, and no more coordination than generating a BLS signing key already requires today. A sender (or key publisher) encrypts an ST toward the *entire* current validator set at once, rather than toward one chosen key holder. Encryption internally samples a small random committee of validators — its size independent of the total validator count — and threshold-shares the DEM key among the committee, such that any sufficiently large subset can recover it while any smaller subset learns nothing. Which validators ended up in a given ST's committee is hidden from everyone, including the ST's own sender, until the ciphertext is public; robustness further guarantees that decryption succeeds even if some committee members submit malformed or missing shares, as long as enough honest members respond.

Mapped onto this EIP's mechanics:

* **No `key_publisher` field.** `Enc` targets the current validator set directly, so there is no party for a sender to select or trust ahead of time, and no `key_publisher` / `key_publication_fee_recipient` / `key_publication_fee` fields are needed on the ST ticket.
* **Reveal becomes a validator duty rather than a discretionary act.** Once an ST-commitment is included in the beacon block, `PartDec` is a fixed, deterministic function of a validator's own key and the (now public) `ciphertext_envelope`. There is no remaining choice to price: a validator's own partial decryption reveals nothing about the plaintext on its own, so there is no incentive to delay it strategically, unlike the withholding described under "Exogenous MEV". Publishing it can be folded into the existing attestation duty and penalized identically to a missed attestation.
* **`LucidKeyMessage` becomes an aggregation, not one party's message.** Instead of a single publisher signing and releasing `k_dem`, the validators sampled into an ST's committee gossip their partial decryptions; any node reconstructs `k_dem` once enough (a fixed threshold, independent of the validator set size) valid partial decryptions have been observed for that ST.
* **Key-timeliness votes count shares, not publishers.** `LucidKeyTimelinessVote` no longer asks whether a specific key publisher's key was seen by the deadline; it asks whether enough valid partial decryptions were observed for a given ST-commitment. Because committee membership is hidden until after commitment, a PTC coalition has no named party to target in advance — the "PTC influence and state observation" bribery vector requires corrupting or colluding with a threshold-sized subset of an a-priori-unknown committee, not a few votes favoring one known address.
* **No sponsor-ST liability workaround needed.** As long as enough honest committee members respond, decryption succeeds regardless of what the remaining members submit — a robustness property of the scheme itself, not an economic disincentive layered on top. This replaces the out-of-protocol liability approximation described under "Key publisher liability for non-reveal" with a cryptographic guarantee: there is no single liable party left to sponsor or penalize, because no single party can cause non-reveal.

This is not proposed as a change to the baseline specification in this revision. Several trade-offs need further work before it could be:

* **Ciphertext overhead scales with committee size, not message size.** At parameters supporting roughly a `t < n/3` corruption bound, per-ST overhead is on the order of tens to hundreds of KB, dominated by committee bookkeeping rather than the (typically small) plaintext transaction. This is workable for a single large payload but not as a per-ST cost in a live mempool; practical adoption would need to amortize one committee sample across a batch of STs, in the spirit of batched threshold-encryption constructions built for this exact mempool-privacy use case. See "Committee sizing and gas budget" below for concrete figures against this EIP's own constants.
* **CCA1, not CCA2.** The construction achieves CCA1-style privacy, one step short of the CCA2 security this EIP's Motivation identifies as desirable.
* **The post-quantum instantiation has a real gap.** It depends on an isogeny-based non-interactive key exchange with no mature implementation. A fallback — each validator trial-decrypting every entry in the committees it might belong to — avoids that dependency but adds per-slot computation that would need to be budgeted against Ethereum's slot timing.
* **One-shot, not fully adaptive, security.** The scheme is proven secure against an adversary that corrupts parties adaptively based on the ciphertext, but only in a single round after it is published. This matches LUCID's single commit-then-reveal transition per ST reasonably well, but it is a real restriction to note rather than full adaptive security.
* **Removing `key_publisher` is a normative change to the ST ticket** and would need to be sequenced against bundle formation (which currently requires a shared `key_publisher` across bundled STs) and against the frame-transaction integration described above.

Given these open points, this is best pursued as a future revision or a parallel `schemeID` variant rather than folded into the baseline specification here.

#### Committee sizing and gas budget

Quantifying the ciphertext-overhead trade-off above against this EIP's own existing constants gives a concrete picture of what committee sizes, and how much per-slot capacity, are actually achievable.

The underlying scheme requires committee size `m ≥ κ / (2ε² · log2(e))` to make its statistical security term negligible, where `κ` is the statistical security parameter and `ε` is the gap between the privacy and reconstruction thresholds around a center `c` (i.e., `t_p = (c-ε)n`, `t_r = (c+ε)n`). At `κ=40` and `c=1/2`: `ε=1/6` (for a `t < n/3` corruption bound) requires `m ≥ 500`; `ε=1/4` (for `t < n/4`) requires `m ≥ 222`.

This EIP already fixes a per-ST byte ceiling, `MAX_BYTES_PER_ST = 2**24 // 64 = 262,144` bytes. Any committee-driven ciphertext must fit under it, which turns out to be the binding constraint on achievable committee size rather than an independent design choice. For a representative plaintext transaction of `B≈500` bytes:

* **Classical instantiation** (secp256k1 PKE + NIKE, ≈142 bytes per committee entry): `m` up to roughly 1,840 fits within `MAX_BYTES_PER_ST`. The `m=500` committee needed for a `t < n/3` bound uses only ≈71.6 KB, comfortably under the cap.
* **Post-quantum instantiation** (ML-KEM-512, no NIKE — using the trial-decryption fallback above, ≈860 bytes per committee entry): only `m` up to roughly 304 fits within `MAX_BYTES_PER_ST`. **The `m=500` committee needed for a `t < n/3` bound does not fit** — it requires ≈430.5 KB, about 1.6x the existing cap. Solving in the other direction, `m=304` at `κ=40` only supports `ε≈0.214`, i.e., a `t < 0.286n` bound — short of `t < n/3`, though better than `t < n/4`.

Reaching `t < n/3` in the post-quantum instantiation without weakening the corruption bound would require raising `MAX_BYTES_PER_ST` to roughly 430 KB, whether generally or specifically for this `schemeID`. This leaves a choice between raising the byte cap, accepting a somewhat weaker post-quantum corruption bound, or amortizing the committee cost across a batch of STs (as noted above) as an open parameter decision rather than one this section resolves.

Translating ciphertext size into gas cost against this EIP's existing budget gives a sense of achievable per-slot throughput. The relevant budget is `tob_gas_limit = block.gas_limit // TOB_GAS_FRACTION_DENOMINATOR`, already reserved for decrypted ToB transactions; at an illustrative `block.gas_limit = 300,000,000`, `tob_gas_limit = 37,500,000`. Since `dual_gas_used` is computed from `max(len(ciphertext_envelope), len(plaintext_tx.data))`, the committee overhead directly drives the calldata-equivalent charge for each ST. Using a range of 16 gas/byte (the standard non-zero calldata byte rate) to 40 gas/byte (accounting for a calldata-floor-pricing mechanism potentially binding, since these ciphertexts are large-payload/low-compute — precisely the shape such mechanisms target; how it would interact with `dual_gas_used` is itself an open question) and the same representative `B≈500` byte transaction:

| Instantiation | Committee `m` | Ciphertext size | Gas per ST | STs per slot (37.5M budget) |
|---|---|---|---|---|
| Classical, `t<n/3` | 500 | ≈71.6 KB | 1.14M – 2.86M | ≈13 – 32 |
| Post-quantum, cap-bound (`t<0.286n`) | 304 | ≈262 KB (at cap) | 4.19M – 10.48M | ≈3 – 8 |
| Post-quantum, `t<n/3` (needs raised cap) | 500 | ≈430.5 KB | 6.89M – 17.22M | ≈2 – 5 |

Even in the cheaper classical instantiation, a single committee-encrypted ST can consume a double-digit percentage of the dedicated ToB budget, and post-quantum instantiations are markedly more constrained. Unbatched, this scheme supports on the order of single digits to a few dozen committee-encrypted STs per slot, not general-purpose mempool throughput — reinforcing that batching a single committee sample across many STs, rather than sampling one committee per ST, is likely a precondition for this design being practical rather than merely an optimization.
