...
**Key generation.** Once seated, the `KRC_SIZE` members generate that period's keypair by acting as both dealer and recipient of a Pedersen-style DKG: each member `i` privately samples a random degree-`(t-1)` polynomial `f_i` and privately shares an evaluation with every other member. No dealer's own contribution, and no coalition smaller than `KRC_RECONSTRUCTION_THRESHOLD`, ever yields a usable secret; the period's group secret `s = Σ_i f_i(0)` exists only implicitly, as an unassembled sum, and its public key `PK_period = [s]` is computed from public per-dealer contributions alone, as described below.
...
`pubkey_contribution_proof` is checked with the same pairing equation [`verify_kzg_proof()`](https://github.com/ethereum/consensus-specs/blob/86fb82b221474cc89387fa6436806507b3849d88/specs/deneb/polynomial-commitments.md#verify_kzg_proof) already evaluates, but against `pubkey_contribution` supplied directly as a group element rather than derived from a scalar the verifier is given — a standard variant that lets a dealer prove its contribution is the correct evaluation of its own committed polynomial at zero without ever revealing that evaluation, `f_i(0)`, in the clear. `PK_period = Σ_i pubkey_contribution_i` (a sum of group elements) is then computable from the broadcast `KRCDealingCommitment`s alone. Each recipient `j`, by contrast, does learn its own raw share `f_i(j)` (confidentially, via `wrapped_share`) and verifies it with `verify_kzg_proof()` exactly as already defined for blob proofs, treating its own seat index as the evaluation point. Neither check requires any party to reference `τ`, the ceremony's secret evaluation point; only the public setup elements already embedded in every client for blob verification are consumed, exactly as they already are today. Dealers may reuse the existing FK20 proof-generation technique, already implemented for blob proofs, to compute their `KRC_SIZE − 1` recipient openings efficiently.

A fully non-interactive DKG additionally requires `wrapped_share` to be publicly verifiable as a correct encryption without decryption, removing the need for an interactive complaint-and-justify round when a recipient's share fails to verify. This section does not specify that composition; it is the subject of an active line of "aggregatable" DKG research built for large, Ethereum-validator-like committees (e.g. ETHDKG-style constructions), and should be resolved by adopting a specific published construction rather than by ad hoc composition here.
...
No `key_publisher`, `key_publication_fee_recipient`, or `key_publication_fee` fields are needed on the ST ticket in this scheme; in their place it carries `krc_period`, identifying which generation's public key was targeted. This is a normative change to the ST ticket, and would need to be sequenced against bundle formation (which otherwise requires a shared `key_publisher` across bundled STs) and against the frame-transaction integration described above. Bundling requires all member tickets to share the same `krc_period`.
...
**Overhead.** The DKG dealing round is paid once per `EPOCHS_PER_KRC_PERIOD`, not once per ST:

| Dealing-round bandwidth (whole committee, once per rotation) | Feldman | KZG (this section) |
|---|---|---|
| Broadcast commitment data | ≈8.4 MB | ≈72 KB |
| Pairwise share + proof data | ≈20.9 MB | ≈33.5 MB |
...
* The EIP-4844 structured reference string is reused only as a public commitment tool; its secret evaluation point `τ` is never used, referenced, or reconstructed by any party in this design. The security of both blob commitments and the dealing round above rests on the same fact — that `τ` is unknown to anyone — and reusing the same public setup for a second application does not weaken that fact or introduce a new trust assumption. This instantiation's exposure to a hypothetical `τ` leak is bounded to the active window of a single dealing round, narrower than blob verification's continuous exposure. Verification contexts should still be domain-separated so a proof produced for one application is never accepted for the other.
...
* Blast radius is per rotation period, not per ST: compromising `KRC_RECONSTRUCTION_THRESHOLD` members at any point while a generation is active exposes every ST encrypted under that generation's key, not only one. This is the cost of amortizing the DKG's setup expense across many STs, and the main reason to prefer a shorter `EPOCHS_PER_KRC_PERIOD` over a longer one, all else equal.
