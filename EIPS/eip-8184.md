...
* The single-key bundles can be useful for more elaborate schemes, for example under threshold decryption; see "Key Reveal Committee: a DKG-based instantiation" below for one such scheme.
...
#### Key Reveal Committee: a DKG-based instantiation

The baseline design gives a single party — either a chosen `key_publisher` or a self-decrypting sender — full discretion over whether to release a `LucidKeyMessage`. As discussed in "PTC influence and state observation" and "Key publisher liability for non-reveal," this concentrates both the risk of non-reveal and the target of any bribery attempt on one identifiable party. This subsection sketches an alternative in which no single party holds that discretion: a rotating, explicitly-selected committee of validators — the **Key Reveal Committee (KRC)** — jointly holds a threshold decryption capability instead.

The most direct way to build such a committee has the sender secret-share each ST's `k_dem` directly across the committee's members. This is workable, but its ciphertext size scales with committee size: reaching a committee large enough to make a corrupt-majority coalition implausible (several hundred members, comparable to `PTC_SIZE`) pushes per-ST overhead into the tens of kilobytes even with efficient encryption, competing with the modest budget this EIP allots the dedicated ToB segment. This instantiation instead has the committee generate and hold a single, standing keypair via a distributed key generation (DKG) protocol, so that per-ST ciphertext size does not depend on committee size at all — at the cost of KRC membership being explicit and known in advance rather than hidden, and, in the specific instantiation below, of a credible post-quantum path.

**Registration and committee selection.** Validators register a persistent key once:

```python
class KeyRevealRegistration(Container):
    validator_index: ValidatorIndex
    krc_pubkey: Bytes48   # a BLS12-381-based encryption key, kept separate from the validator's signing key
```

```python
KRC_SIZE = uint64(2**9)                     # 512, matching PTC_SIZE
KRC_RECONSTRUCTION_THRESHOLD = uint64(342)  # ~2/3 of KRC_SIZE
DOMAIN_KEY_REVEAL_COMMITTEE = DomainType('TBD')
DOMAIN_PARTIAL_DECRYPTION = DomainType('TBD')
```

`get_key_reveal_committee(state, period)` selects `KRC_SIZE` seats from the active, registered validator set using the same shuffling approach already used for `get_ptc` and the sync committee, seeded via `get_seed(state, ..., DOMAIN_KEY_REVEAL_COMMITTEE)`. `KRC_SIZE = 512` matches `PTC_SIZE`: since the KRC is itself a committee randomly sampled from the validator set, its own corruption ratio must concentrate near the validator set's assumed global bound with high confidence, and a committee of this size carries the same statistical margin Ethereum already relies on for the sync committee and PTC. Committees rotate every `EPOCHS_PER_KRC_PERIOD`, a tunable parameter on the order of a handful of epochs up to roughly a day: shorter periods reduce the bribery-exposure window and the cost of an eventual compromise (see below), at the price of more frequent dealing rounds.

**Key generation.** Once seated, the `KRC_SIZE` members generate that period's keypair by acting as both dealer and recipient of a Pedersen-style DKG: each member `i` privately samples a random degree-`(t-1)` polynomial `f_i` and privately shares an evaluation with every other member. No dealer's own contribution, and no coalition smaller than `KRC_RECONSTRUCTION_THRESHOLD`, ever yields a usable secret; the period's group secret `s = Σ_i f_i(0)` exists only implicitly, as an unassembled sum, and its public key `PK_period = [s]` is computed from public per-dealer contributions alone, as described below. One known caveat of Pedersen-style DKGs applies: a rushing adversary can bias the distribution of the resulting public key (Gennaro, Jarecki, Krawczyk and Rabin, "Secure Distributed Key Generation for Discrete-Log Based Cryptosystems", 1999). The bias is tolerable for many threshold-encryption applications, but the adopted construction must either eliminate it or explicitly argue it harmless in this setting.

A textbook Feldman-based DKG would have each dealer publish `t` per-coefficient commitments — `O(KRC_SIZE · t)` broadcast data across the committee, several megabytes at these parameters. This instantiation instead has each dealer publish a single `KZGCommitment` to its polynomial `f_i`, reusing the structured reference string already published for [EIP-4844](./eip-4844.md), together with its own contribution to the group public key and a proof binding the two:

```python
class KRCDealingCommitment(Container):
    krc_period: uint64
    dealer_seat: uint16
    commitment: KZGCommitment            # commits to dealer i's degree-(t-1) polynomial f_i
    pubkey_contribution: Bytes48         # dealer i's contribution to PK_period, as a group element rather than the raw scalar f_i(0)
    pubkey_contribution_proof: KZGProof  # binds pubkey_contribution to commitment at the point 0

class KRCDealingShare(Container):
    krc_period: uint64
    dealer_seat: uint16
    recipient_seat: uint16
    wrapped_share: ByteList[512]   # encrypts (f_i(j), KZGProof) to seat j's krc_pubkey
```

`pubkey_contribution_proof` is checked with the same pairing equation [`verify_kzg_proof()`](https://github.com/ethereum/consensus-specs/blob/86fb82b221474cc89387fa6436806507b3849d88/specs/deneb/polynomial-commitments.md#verify_kzg_proof) already evaluates, but against `pubkey_contribution` supplied directly as a group element rather than derived from a scalar the verifier is given — a standard variant, used in aggregatable DKG constructions (Gurkan, Jovanovic, Kokoris-Kogias, Nikolaenko and Tomescu, "Aggregatable Distributed Key Generation", 2021), that lets a dealer prove its contribution is the correct evaluation of its own committed polynomial at zero without ever revealing that evaluation, `f_i(0)`, in the clear. `PK_period = Σ_i pubkey_contribution_i` (a sum of group elements) is then computable from the broadcast `KRCDealingCommitment`s alone. Each recipient `j`, by contrast, does learn its own raw share `f_i(j)` (confidentially, via `wrapped_share`) and verifies it with `verify_kzg_proof()` exactly as already defined for blob proofs, at the evaluation point assigned to its seat. Neither check requires any party to reference `τ`, the ceremony's secret evaluation point; only the public setup elements already embedded in every client for blob verification are consumed, exactly as they already are today. Dealers may apply the FK20 batch-opening technique to compute their `KRC_SIZE − 1` recipient openings efficiently, though not through the blob code paths as-is: blob proofs open degree-4095 polynomials over a fixed 4096-point roots-of-unity domain, whereas dealing opens degree-`(t-1)` polynomials at `KRC_SIZE` seat points. Seat evaluation points are accordingly assigned from a power-of-two roots-of-unity domain of size `KRC_SIZE`, with the point-0 opening backing `pubkey_contribution_proof` computed separately, since 0 is not an element of a multiplicative evaluation domain.

A fully non-interactive DKG additionally requires `wrapped_share` to be publicly verifiable as a correct encryption without decryption, removing the need for an interactive complaint-and-justify round when a recipient's share fails to verify. This section does not specify that composition; it is the subject of an active line of "aggregatable" DKG research built for large, Ethereum-validator-like committees (e.g. the aggregatable DKG of Gurkan et al. cited above, or ETHDKG-style constructions), and should be resolved by adopting a specific published construction rather than by ad hoc composition here.

**Encryption and reveal.** Encrypting to `PK_period` is ordinary ElGamal-style encryption and requires no committee interaction:

```python
class KRCEnvelopeHeader(Container):
    scheme_id: uint8
    krc_period: uint64
    U: Bytes48             # an ephemeral KEM value, fresh per ST
    wrapped_k_dem: Bytes48 # k_dem (32 bytes) plus AEAD tag, keyed by a KDF of the shared secret derived from U and PK_period
```

Reveal keeps the baseline design's timing, distributed across the committee rather than held by one party. Once the ST-commitment referencing `krc_period` is observed included and attested in the beacon block — the same trigger the baseline key publisher already waits for — each seat `j` of that period's KRC publishes a partial decryption computed from `U` and its own share `sk_j = Σ_i f_i(j)`, without reconstructing `s`:

```python
class PartialDecryption(Container):
    chain_id: uint256
    scheduling_beacon_block_root: Bytes32
    scheduling_slot: uint64
    commit_index: uint8
...
class SignedPartialDecryption(Container):
    message: PartialDecryption
    signature: BLSSignature
```

Once `KRC_RECONSTRUCTION_THRESHOLD` valid partial decryptions are observed for a commitment, any node combines them by scalar-multiplying each by its corresponding public Lagrange coefficient and summing the results — the same interpolation used to reconstruct a Shamir secret, applied here to each member's partial decryption rather than to its raw share, so the combiner recovers the shared secret without ever recovering `s` itself. From there, `k_dem` is unwrapped exactly as from a baseline `LucidKeyMessage`. `key_commitment`, `reveal_commitment`, and the entirety of "Key message validity and decryption," "Key timeliness," and "Recovery under rejected payload" apply unchanged, since they depend only on whether a valid `k_dem` was observed by the relevant deadline, not on how it was produced.

**Message transport.** Each message class above has a distinct distribution model, chosen so that per-slot global gossip stays within the budget of existing beacon topics:

* `KRCDealingCommitment`s are broadcast on a global gossip topic and additionally included in beacon blocks as that period's dealing transcript. This makes `PK_period` derivable from chain data alone, gives late-joining or resyncing members a canonical transcript, and fixes the evidence base against which dealing complaints are adjudicated. At ≈72 KB per rotation, spread across a dealing window, this is a modest addition to block data.
* `KRCDealingShare`s are not consensus data and are never globally flooded: each is published to a per-recipient-seat topic (analogous to attestation subnets) to which only the recipient — and any party assisting its retrieval — subscribes.
* `SignedPartialDecryption`s are published to a committee-scoped subnet rather than flooded globally. Global flooding would not scale: 512 partials of ≈230 bytes for each of up to `MAX_ST_COMMITS` commitments approaches several megabytes per slot, comparable to full blob throughput. Any subscriber — committee members, the next proposer, the ST sender, or any other interested node — combines observed partials and floods only the resulting 32-byte `k_dem` globally, as an ordinary `LucidKeyMessage`. Because a combined key self-verifies against `key_commitment`, the wider network's validation path is byte-for-byte the baseline one, and combining is permissionless: withholding by any particular combiner is meaningless, since the first valid combination wins.
* Anti-DoS validation on all three topics is cheap and stateless: messages are dropped unless their signature verifies under the appropriate domain (`DOMAIN_KEY_REVEAL_COMMITTEE` or `DOMAIN_PARTIAL_DECRYPTION`), their `krc_period` is the current or next period, their seat indices are consistent with `get_key_reveal_committee`, and all group elements pass subgroup checks.

The `key_publisher`, `key_publication_fee_recipient`, and `key_publication_fee` fields are not meaningful under this scheme. In their place, the ST ticket carries two purpose-named fields:

* `krc_period` — identifies which generation's public key the sender encrypted to.
* `key_reveal_fee` — the fee the sender pays for a timely reveal, charged exactly as `key_publication_fee` is charged in the baseline, but distributed across the committee seats recorded as participating in the reveal rather than paid to a single recipient.

This is a normative change to the ST ticket, and would need to be sequenced against bundle formation (which otherwise requires a shared `key_publisher` across bundled STs) and against the frame-transaction integration described above. Bundling requires all member tickets to share the same `krc_period`.

**Incentives and attribution.** A threshold of 342-of-512 invites free riding: each seat individually expects the other seats to cover a reveal, so participation must be paid and absence must cost. The block that carries a commitment's decrypted transactions also carries, for that commitment, a `participation_bits: Bitvector[KRC_SIZE]` recording the seats whose valid partial decryptions were observed by the key deadline, analogous to the sync committee's aggregate bits. All recorded seats — not merely the first `KRC_RECONSTRUCTION_THRESHOLD` — split the commitment's `key_reveal_fee` pro rata, so publishing a partial is never individually wasted. Seated validators that persistently fail to appear in `participation_bits` accrue inactivity-style penalties, sized like sync-committee absence penalties: enough to make lazy non-participation irrational, but not slashing-grade, since mere absence is indistinguishable from network failure. Publishing two conflicting signed partial decryptions for the same `(commitment, seat)` pair is attributable equivocation, and is slashable.

The sender-side ToB penalty for non-reveal is deliberately retained even though non-reveal is now a committee failure rather than a sender choice. Waiving or refunding it when fewer than `KRC_RECONSTRUCTION_THRESHOLD` partials appear would let a sender colluding with `KRC_SIZE - KRC_RECONSTRUCTION_THRESHOLD + 1 = 171` seats recover penalty-free reveal optionality — precisely what this instantiation exists to remove. Senders instead price in residual committee-liveness risk, which the participation rewards and penalties above are sized to keep small.

An ST not committed before its `krc_period` rotates out of use is not specially handled: the sender simply re-encrypts `k_dem` to the current period's `PK_period` and resubmits. This instantiation does not propose a proactive resharing protocol to hand custody of an unrevealed ST from an outgoing generation to its successor; the cost of that choice is a bound on ST validity of roughly one rotation period, in exchange for materially simpler rotation.

**Overhead.** The DKG dealing round is paid once per `EPOCHS_PER_KRC_PERIOD`, not once per ST. Dealing data falls in two classes with different cost models: broadcast data is received and verified by every committee member (and by any observer auditing the transcript), while each encrypted share travels point-to-point to exactly one recipient. Costs per rotation:

| Dealing-round bandwidth (once per rotation) | Feldman | KZG (this section) |
|---|---|---|
| Broadcast data, received by every node | ≈8.4 MB | ≈72 KB |
| Share + proof data, received per member | ≈41 KB | ≈65 KB |
| Share + proof data, summed across all members | ≈20.9 MB | ≈33.5 MB |

Summed across the committee, the two variants transfer comparable total bytes — KZG in fact slightly more, since each KZG share embeds its own 48-byte opening proof while Feldman shares verify against the broadcast coefficient commitments. The advantage lies in where the bytes land: per node, dealing costs ≈8.4 MB under Feldman versus ≈140 KB under KZG, a ~60x reduction, because the per-coefficient broadcast vector is replaced by a single commitment. Verification compute follows the same shape: a KZG recipient checks two pairings per share received, while a Feldman recipient computes a `t`-term multi-scalar multiplication per share. One caveat binds this analysis to the transport model: if dealing later adopts a fully publicly-verifiable construction in which encrypted shares are posted publicly to enable non-interactive complaint resolution, pairwise data becomes broadcast data and the summed totals govern instead; the choice of published DKG construction should be made with this trade-off in view.

Per ST, this instantiation carries a fixed `KRCEnvelopeHeader` payload of roughly 100 bytes regardless of `KRC_SIZE`. The direct alternative noted above — the sender secret-sharing `k_dem` across all `KRC_SIZE` members individually — needs a distinct encrypted share per member, so its ciphertext grows linearly with committee size; at `KRC_SIZE = 512` this reaches tens of kilobytes per ST even with efficient encryption, a meaningful fraction of the gas this EIP allots the dedicated ToB segment. This instantiation avoids that scaling entirely by moving the committee-size-dependent cost to the periodic DKG dealing round above rather than to each ST's ciphertext.

**Security Considerations for this instantiation.**

* The EIP-4844 structured reference string is reused only as a public commitment tool; its secret evaluation point `τ` is never used, referenced, or reconstructed by any party in this design. The security of both blob commitments and the dealing round above rests on the same fact — that `τ` is unknown to anyone — and reusing the same public setup for a second application does not weaken that fact or introduce a new trust assumption. Exposure to a hypothetical `τ` leak is bounded to keys generated by dealing rounds conducted while `τ` was known: a dealer holding `τ` could forge openings that make inconsistent shares verify — an attack mounted during dealing whose damage, partial decryptions that fail to interpolate, manifests later at reveal time. Keys from dealing rounds completed before a leak remain sound. Domain separation cannot live in the pairing check itself, which is unauthenticated algebra; it is enforced at the message layer: every dealing and partial-decryption envelope is signed under the dedicated `DOMAIN_KEY_REVEAL_COMMITTEE` or `DOMAIN_PARTIAL_DECRYPTION` type, so no signed message produced for one context is acceptable in another.
* This instantiation has no credible post-quantum path: both the DKG's internal algebra and the ElGamal-style encryption to `PK_period` rely on discrete-log/pairing hardness over BLS12-381. This is an explicit, accepted trade-off in exchange for a constant per-ST ciphertext and reuse of curve and proof-system infrastructure already relied upon for validator signatures and blob commitments, and should be weighed against the credible-quantum-safety property this EIP's Motivation identifies as desirable if this instantiation is considered for anything beyond an optional `schemeID`.
* Blast radius is per rotation period, not per ST, and the scheme's two coalition scales should be read together: `KRC_SIZE - KRC_RECONSTRUCTION_THRESHOLD + 1 = 171` seats suffice to withhold a reveal — a liveness failure that is attributable by omission against the signed partials of the seats that did publish — while `KRC_RECONSTRUCTION_THRESHOLD = 342` seats suffice to decrypt every pending ST of the period before commitment, silently, leaving no onchain trace. The comparison to the baseline is therefore not uniformly favorable and should be stated plainly: a sender-chosen `key_publisher` can always read the plaintexts routed to it, but that trust is opt-in, reputational, and partitioned per publisher, and a withholding publisher fails visibly; a self-decrypting sender exposes nothing to anyone. The KRC replaces per-publisher, opt-in trust with systemic, protocol-assigned trust whose confidentiality failure mode is undetectable. Shortening `EPOCHS_PER_KRC_PERIOD` shrinks the exposure window and the value at risk per generation, but does not change either coalition threshold. This exposure profile is the cost of amortizing the DKG's setup expense across many STs, and the main reason to prefer a shorter `EPOCHS_PER_KRC_PERIOD` over a longer one, all else equal.

Open points before this could be proposed as a baseline change: a specific published construction for fully non-interactive, publicly-verifiable dealing rather than a complaint-and-justify round; a chosen `EPOCHS_PER_KRC_PERIOD`; and whether `krc_pubkey` should be a dedicated, separately-registered key or a domain-separated derivation of a validator's existing signing key.
